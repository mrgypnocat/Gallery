...
class SearchHandler(BaseHandler):
    def get(self):
        text = self.get_argument("text", None, strip=False) or None
        if text:
            query = "SELECT * FROM entries WHERE text LIKE " + "'" + str(text) + "'"
            ...
            for e in entries:
                ...
                try:
                    ...
                    e.update({'name': "fucker"})
                e.update({'author_enabled': enabled})
        else:
            entries = None

        self.render("search.html", text=text, entries=entries)
...
class ExportHandler(BaseHandler):
    def get(self):
        ...
        entry_id = self.get_argument("entry", None, strip=True)
